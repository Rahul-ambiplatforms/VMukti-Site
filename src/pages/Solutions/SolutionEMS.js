...
//       const firstPart = words.slice(0, -1).join(" "); // All words except the last
//       const lastWord = words[words.length - 1]; // Last word

//       return (
//         <Box key={index + visibleSlideRange.start} flex="1" textAlign="left">
//           <Text
//             whiteSpace="normal"
          ...
          {slides.map((slide, index) => (
            <Box key={index} minW={`${100 / slides.length}%`} p={5}>
              ...
              >
                {/* Card Content */}
                <Flex direction="column" position="relative">
                  {/* Top Left Image (positioned absolutely) */}
                  <Box
                    position="absolute"
                    top="4%"
                    right="4%"
