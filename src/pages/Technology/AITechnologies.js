...
const AITechnologies = () => {
  ...
  //     const screenWidth = window.innerWidth;
      ...
      // }else if (visibleSlides == 5) {
      //   if (currentSlide < visibleSlideRange.start) {
      //     setVisibleSlideRange({ start: currentSlide, end: currentSlide + 4 });
      //   } else if (currentSlide > visibleSlideRange.end) {
      //     setVisibleSlideRange({ start: currentSlide - 4, end: currentSlide });
      //   }
      // }
  //   };
...
//   useEffect(() => {
//     const initialEnd = Math.min(visibleSlides - 1, slides.length - 1);
//     setVisibleSlideRange({ start: 0, end: initialEnd });
//   }, [visibleSlides, slides.length]);
  
//   // Update visible range with loop handling
//   useEffect(() => {
//     if (!visibleSlides || !slides.length) return;
  
//     const isLoopingForward = prevSlideRef.current === slides.length - 1 && currentSlide === 0;
//     const isLoopingBackward = prevSlideRef.current === 0 && currentSlide === slides.length - 1;
  
//     if (isLoopingForward) {
//       // Jump to beginning
//       setVisibleSlideRange({
//         start: 0,
//         end: Math.min(visibleSlides - 1, slides.length - 1)
//       });
//     } 
//     else if (isLoopingBackward) {
//       // Jump to end
//       setVisibleSlideRange({
//         start: Math.max(slides.length - visibleSlides, 0),
//         end: slides.length - 1
//       });
//     }
//     else {
//       // Normal navigation
//       const isOutOfViewRight = currentSlide > visibleSlideRange.end;
//       const isOutOfViewLeft = currentSlide < visibleSlideRange.start;
  
//       if (isOutOfViewRight) {
//         setVisibleSlideRange(prev => ({
//           start: prev.start + 1,
//           end: Math.min(prev.end + 1, slides.length - 1)
//         }));
//       }
