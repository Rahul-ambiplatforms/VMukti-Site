...
const BlogPreview = ({ formData, components, faqTitle }) => {
  ...
  const getImageSource = (file) => {
    ...
    if (typeof file === 'string') {
      // If it's a path starting with /uploads, prepend the backend URL
  ...
  useEffect(() => {
    ...
    if (formData.mainImage) {
      if (formData.mainImage instanceof File) {
        ...
        // If it's a path starting with /uploads, prepend the backend URL
        if (formData.mainImage.startsWith('/uploads')) {
          setMainImageUrl(`http://localhost:5000${formData.mainImage}`);
        } else {
    ...
    <Container maxW="container.xl" p={0}>
      ...
      <VStack spacing={8} align="stretch">
        {components.reduce((groups, component, index) => {
          ...
          // Start a new group for headings
                  ...
                  return (
                    <Box key={component.id} my={4}>
                      {(component.content.file || component.content.url || component.content.imagePath) ? (
                        <Image
                          src={getImageSource(component.content.file || component.content.url || component.content.imagePath)}
